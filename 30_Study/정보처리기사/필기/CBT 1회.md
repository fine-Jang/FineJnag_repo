...
3. 익스트림 프로그래밍(eXtreme Programming)의 5가지 가치에 속하지 않는 것은?
4) 의사소통
5) 단순성
6) 피드백
7) 고객 배제
...
8. 모바일 기기에서 사용하는 NUI 인터페이스에 속하지 않는 것은 무엇인가?
9) Pinch
10) Press
11) Flow
12) Flick
...
18. 보헴이 제안한 것으로, 위험 분석 기능이 있으며, 여러 번의 개발 과정을 거쳐 점진적으로 개발하는 모형은?
19) 나선형 모형
20) 애자일 모형
21) 프로토타입 모형
22) 폭포수 모형
...
23. UML에서 시퀀스 다이어그램의 구성 항목에 해당하지 않는 것은?
24) 생명선
25) 실행
26) 확장
27) 메시지
...
23.소프트웨어 형상 관리(Configuration management)에 관한 설명으로 틀린 것은?
...
토막강의 보기
35.소스 코드 품질 분석 도구 중 정적 분석 도구가 아닌 것은?
1)pmd
2)checkstyle
3)valance
4)cppcheck
...
36.알파, 베타 테스트와 가장 밀접한 연관이 있는 테스트 단계는?
1)단위 테스트
2)인수 테스트
3)통합 테스트
4)시스템 테스트
...
60.데이터베이스에 영향을 주는 생성, 읽기, 갱신, 삭제 연산으로 프로세스와 테이블 간에 매트릭스를 만들어서 트랜잭션을 분석하는 것은?
1)CASE 분석
2)일치 분석
3)CRUD 분석
4)연관성 분석
...
92.SQL Injection 공격과 관련한 설명으로 틀린 것은?
