...
## 9. 다음 자료에 대하여 선택(Selection) 정렬을 이용하여 오름차순으로 정렬하고자 한다. 1회전 수행 결과는?
...
## 11. 순서가 A, B, C, D로 정해진 입력 자료를 스택에 입력하였다가 출력할 때, 가능한 출력 순서의 결과가 아닌 것은?
...
## 12. 소스 코드 품질 분석 도구 중 정적 분석 도구가 아닌 것은?
...
## 13. 알파, 베타 테스트와 가장 밀접한 연관이 있는 테스트 단계는?
...
## 14. 연결 리스트(Linked List)에 대한 설명으로 거리가 먼 것은?
...
## 15. 관계대수와 관계해석에 대한 설명으로 옳지 않는 것은?
...
## 16. 데이터베이스에 영향을 주는 생성, 읽기, 갱신, 삭제 연산으로 프로세스와 테이블 간에 매트릭스를 만들어서 트랜잭션을 분석하는 것은?
...
## 18. 다음 C언어 프로그램에서 밑줄 친 부분과 동일한 의미를 가지는 것은 어떤것인가?
...
## 20. C언어에서 malloc( ) 함수에 대한 설명으로 틀린 것은?
...
## 24. 프레임워크(Framework)에 대한 설명으로 옳은 것은?
...
## 25. C언어 라이브러리 중 stdlib.h에 대한 설명으로 옳은 것은?
...
## 26. 악성코드의 유형 중 다른 컴퓨터의 취약점을 이용하여 스스로 전파하거나 메일로 전파되며 스스로를 증식하는 것은?
...
## 27. 정보 보안 요소 중 무결성(Integrity)에 대한 설명으로 옳은 것은?
...
DES의 보안 문제 해결을 위해 NIST에서 개발한 개인키 암호화 알고리즘은 AES(Advanced Encryption Standard)입니다.
...
## 30. 침입 탐지 시스템(IDS; Intrusion Detection System)과 관련한 설명으로 틀린 것은?
...
## 32. SQL Injection 공격과 관련한 설명으로 틀린 것은?
...
## 33. 소프트웨어 재공학의 주요 활동 중 기존 소프트웨어를 다른 운영체제나 하드웨어 환경에서 사용할 수 있도록 변환하는 것은?
...
## 35. CPM(Critical Path Method)에 대한 설명으로 옳지 않은 것은?
